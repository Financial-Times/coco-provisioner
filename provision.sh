...
. .venv/bin/activate && echo $VAULT_PASS > /vault.pass && ansible-playbook -i ~/.ansible_hosts /ansible/aws_coreos_site.yml  \
  --extra-vars " \
  token=$TOKEN_URL \
  ...
  --extra-vars "@user_config.json" --vault-password-file=/vault.pass
